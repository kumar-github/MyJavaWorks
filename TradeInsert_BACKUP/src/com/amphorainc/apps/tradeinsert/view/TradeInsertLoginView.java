...
public class TradeInsertLoginView extends JDialog
{
	...
	public void showGUIIfNeeded()
	{
		/*
		 * decide here whether to show the login screen or not. if we show the login screen, then as per the user action (ok/cancel) the flow goes. 
		 * if we are not showing the login screen for some reason, still it should authenticate the user by taking the credentials from the file and 
		 * not from the UI.
		 */
	...
	*/
	...
	private boolean shouldShowLoginScreen()
	{
		...
		boolean shouldShowLoginScreen = true;
		try
		{
